...
import { Smartphone, Globe, Phone, CreditCard, ArrowRight } from "lucide-react";

interface AirtimeTabProps {
    walletBalance?: number;
}

const AirtimeTab: FC<AirtimeTabProps> = ({ walletBalance }) => {
    const [amount, setAmount] = useState<string>("");
    const [selectedCountry, setSelectedCountry] = useState<string>("");
    const [selectedProvider, setSelectedProvider] = useState<string>("");

    // Calculate the estimated airtime based on amount (assuming 1:1 conversion)
    const serviceFee = 0.05;
    const estimatedAirtime = amount ? parseFloat(amount) - serviceFee : 0;
    ...
    return (
        ...
        >
            <motion.div variants={itemVariants} className="mb-8">
                <h1 className="text-2xl font-bold text-slate-900 dark:text-white mb-1 bg-gradient-to-r from-blue-600 via-teal-500 to-green-500 bg-clip-text text-transparent">Convert to Airtime</h1>
                <p className="text-slate-500 dark:text-slate-400">Convert your RPSC tokens to mobile airtime</p>
            </motion.div>
            ...
            <Card className="overflow-hidden border-0 shadow-lg relative">
                ...
                <CardHeader className="relative z-10">
                    <div className="flex items-center space-x-3 mb-2">
                        ...
                        <div>
                            <CardTitle>Convert to Airtime</CardTitle>
                            <CardDescription>Choose provider and amount to convert</CardDescription>
                        </div>
                    </div>
                </CardHeader>
                ...
                <CardContent className="space-y-6 relative z-10">
                    <motion.div variants={itemVariants} className="space-y-4">
                        <div className="space-y-2">
                            ...
                            <div className="relative">
                                <Select value={selectedCountry} onValueChange={setSelectedCountry}>
                                    <SelectTrigger className="pl-10 border-slate-200 dark:border-slate-700 focus:ring-2 focus:ring-blue-500 dark:focus:ring-blue-400 transition-all">
                        ...
                        <div className="space-y-2">
                            <Label htmlFor="airtimeAmount" className="text-sm font-medium">Amount (RPSC)</Label>
                            <div className="relative">
                                <Input
                                    id="airtimeAmount"
                                    type="text"
                    ...
                    <motion.div variants={itemVariants} className="space-y-4 bg-slate-50 dark:bg-slate-800/40 p-4 rounded-lg">
                        <div className="flex items-center justify-between">
                            ...
                            <span className="font-medium text-slate-900 dark:text-white">1 RPSC = ₦1</span>
                        </div>
                        <div className="flex items-center justify-between">
                            <span className="text-sm text-slate-500 dark:text-slate-400">Service Fee</span>
                            <span className="font-medium text-slate-900 dark:text-white">{serviceFee} RPSC</span>
                        </div>
                        <div className="flex items-center justify-between">
                            <span className="text-sm text-slate-500 dark:text-slate-400">You'll Receive</span>
                            <span className="font-bold text-transparent bg-gradient-to-r from-blue-600 via-teal-500 to-green-500 bg-clip-text text-lg">₦{estimatedAirtime.toFixed(2)}</span>
                        </div>
                    </motion.div>
                </CardContent>
                ...
                <CardFooter className="relative z-10 pt-2 pb-6">
                    ...
                    >
                        ...
                        >
                            <span className="mr-2">Convert to Airtime</span>
